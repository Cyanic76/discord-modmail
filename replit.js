/*
    this file is optimized for repl.it.
*/
...
client.on("message", async message => {
  ...
  if(message.channel.type === "dm"){
    ...
    let user = await db.get(`isBlocked${message.author.id}`);
    ...
    if(active === null){
      ...
      let ctno = await db.get("ticket"); // 1 for example
      await db.set("ticket", ctno+1);
      let actualticket = await db.get("ticket");
    ...
    await db.set(`supportChannel_${active.channelID}`, message.author.id);
    return;
  }
  if(message.author.bot) return;
  var support = await db.get(`supportChannel_${message.channel.id}`);
  let supportServer = client.guilds.cache.get(config.guild);
  if(support){
    var support = await db.get(`support_${support}`);
    let supportUser = client.users.cache.get(support.targetID);
    if(!supportUser) return message.channel.delete();

    // reply (with user and role)
    if(message.content.startsWith(`${config.prefix}reply`)){
      var isPause = await db.get(`suspended${support.targetID}`);
    ...
    if(message.content.toLowerCase() === `${config.prefix}complete`){
      ...
      async function end(userID){
        ...
        .setTimestamp()
        await db.delete(`support_${userID}`);
  ...
  if(message.content.startsWith(`${config.prefix}unblock`)){
    if(message.guild.member(message.author).roles.cache.has(config.roles.mod)){
      ...
      client.users.fetch(`${args[0]}`).then(async user => {
        ...
        if(data === true){
          await db.delete(`isBlocked${args[0]}`);
